function nbv_constructor(document, deps) {
  'use strict';
  ...
  function render_ipynb(obj, target, settings) {
    ...
    for (let j = 0; j < cells.length; j++) {
      ...
      let dm = d.createElement('div'); // empty div as a fallback
  ...
  function handle_code(cell) {
    const el = d.createElement('div'); // container for it all
    ...
    const raw_source = (cell.source || cell.input);
  ...
  function handle_cell_output(dt) {
    ...
    const format_priority = ['text/html', 'image/svg+xml', 'image/png', 'image/jpeg', 'text/plain'];

    let fmt = null;
    for (const tfmt of format_priority) {
    ...
    let dm = d.createElement('div');
    const source = handle_multiline_strings(dt.data[fmt]);
    switch (fmt) {
      case 'text/plain':
      ...
      case 'text/html': {
        ...
        const dfs = dm.getElementsByClassName('dataframe');
        for (let k = 0; k < dfs.length; k++) {
          ...
          const cl = dfs[k].querySelectorAll('td, th');
      ...
      default:
        if (fmt.startsWith('image/')) {
          ...
          if (fmt in (dt.metadata || {})) {
            const metadata = dt.metadata[fmt];
  ...
  function handle_error_cell(dt) {
    const cn = d.createElement('pre');
    let txt = handle_multiline_strings(dt.traceback);
    ...
    txt = txt.replace(/[\u001B\u009B][[()#;?]*(?:\d{1,4}(?:;\d{0,4})*)?[0-9A-ORZcf-nqry=><]/g, '');
    cn.textContent = txt;
  ...
  function handle_stream_output(dt) {
    ...
    const outt = dt.name || dt.stream; // v4 || v3; contains 'stdout' or 'stderr'
  ...
  function handle_mdown(source) {
    const el = d.createElement('div');
    source = handle_multiline_strings(source);
    let latexed = source.replace(/\$\$([\s\S]+?)\$\$/g, latexer(true)); // block-based math
    latexed = latexed.replace(/\$(.+?)\$/g, latexer(false)); // inline math
    el.innerHTML = deps.marked.parse(latexed);
  ...
  function handle_pyout(cell) {
    ...
    Object.keys(cell).forEach((k) => {
      ...
      if (['output_type', 'prompt_number', 'metadata'].includes(k))
        return;
      let p = d.createElement('span'); // if errs
      switch (k) {
        case 'text':
